...
resource "null_resource" "swarm_os_setup" {
  ...
  provisioner "local-exec" {
    command = "ansible -T 30 -u root -m community.docker.docker_swarm -a 'state=present subnet_size=16' --ssh-extra-args='-o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no' --inventory=${var.terraform_inventory_path} swarm_0"
...
resource "null_resource" "swarm_deployer_script" {
  ...
  provisioner "local-exec" {
    command = format("ansible -T 30 -u ${var.username} -m shell -a 'echo \"command=\\\"/home/beamup/beamup-sync-swarm.sh\\\",restrict %s\" >> /home/${var.username}/.ssh/authorized_keys' --ssh-extra-args='-o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no' --inventory=${var.terraform_inventory_path} swarm", file("./id_ed25519_deployer_sync.pub"))
